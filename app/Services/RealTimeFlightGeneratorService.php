...
class RealTimeFlightGeneratorService
{
    ...
    public function generateLiveSchedules(): int
    {
        ...
        $today = Carbon::today();
        $hubCodes = ['PNH', 'SAI', 'SIN', 'BKK', 'JFK', 'CDG', 'HND', 'LHR', 'DXB', 'KUL', 'LAX'];
        $hubs  = $airports->whereIn('code', $hubCodes);

        // Pre-fetch existing flight numbers to avoid duplicates without per-row DB calls
        $existing = Flight::pluck('flight_number')->flip()->toArray();
        ...
        for ($day = 0; $day < 7; $day++) {
            ...
            foreach ($hubs as $hub) {
                ...
                foreach ($destinations as $idx => $dest) {
                    ...
                    $key = $flightNumber . '_' . $dateStr;

                    if (isset($existing[$flightNumber]) || isset($rows[$key])) {
                        continue;
                    }
                    ...
                    $arrCarbon = Carbon::parse($dateStr . ' ' . $depTime)->addMinutes($durationMinutes);
        ...
        // Single bulk insert for all flights
