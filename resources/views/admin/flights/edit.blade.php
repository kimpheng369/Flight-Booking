...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="bg-white rounded-3xl p-8 shadow-sm border border-slate-200/80 space-y-6">
        <h1 class="text-2xl font-extrabold text-slate-900">Edit Flight: {{ $flight->flight_number }}</h1>

        <form method="POST" action="{{ route('admin.flights.update', $flight) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="flight_number" value="{{ old('flight_number', $flight->flight_number) }}" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold">
                ...
                <div class="space-y-1.5">
                    ...
                    <select name="airline_id" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold">
                        @foreach($airlines as $al)
                            <option value="{{ $al->id }}" {{ $flight->airline_id == $al->id ? 'selected' : '' }}>{{ $al->name }} ({{ $al->code }})</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <select name="aircraft_id" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold">
                        @foreach($aircrafts as $ac)
                            <option value="{{ $ac->id }}" {{ $flight->aircraft_id == $ac->id ? 'selected' : '' }}>{{ $ac->model }} ({{ $ac->registration_number }})</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="number" step="0.01" name="price" value="{{ old('price', $flight->price) }}" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold">
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <select name="origin_airport_id" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold">
                        @foreach($airports as $ap)
                            <option value="{{ $ap->id }}" {{ $flight->origin_airport_id == $ap->id ? 'selected' : '' }}>{{ $ap->city }} ({{ $ap->code }})</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <select name="destination_airport_id" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold">
                        @foreach($airports as $ap)
                            <option value="{{ $ap->id }}" {{ $flight->destination_airport_id == $ap->id ? 'selected' : '' }}>{{ $ap->city }} ({{ $ap->code }})</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="date" name="departure_date" value="{{ old('departure_date', \Carbon\Carbon::parse($flight->departure_date)->format('Y-m-d')) }}" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold">
                </div>

                <!-- Departure Time -->
                <div class="space-y-1.5">
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-600">Departure Time</label>
                    <input type="time" name="departure_time" value="{{ old('departure_time', \Carbon\Carbon::parse($flight->departure_time)->format('H:i')) }}" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold">
                </div>

                <!-- Arrival Date -->
                <div class="space-y-1.5">
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-600">Arrival Date</label>
                    <input type="date" name="arrival_date" value="{{ old('arrival_date', \Carbon\Carbon::parse($flight->arrival_date)->format('Y-m-d')) }}" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold">
                </div>

                <!-- Arrival Time -->
                <div class="space-y-1.5">
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-600">Arrival Time</label>
                    <input type="time" name="arrival_time" value="{{ old('arrival_time', \Carbon\Carbon::parse($flight->arrival_time)->format('H:i')) }}" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold">
                </div>

                <!-- Total Seats -->
                <div class="space-y-1.5">
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-600">Total Seats</label>
                    <input type="number" name="total_seats" value="{{ old('total_seats', $flight->total_seats) }}" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold">
                </div>

                <!-- Available Seats -->
                <div class="space-y-1.5">
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-600">Available Seats</label>
                    <input type="number" name="available_seats" value="{{ old('available_seats', $flight->available_seats) }}" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold">
                </div>

                <!-- Baggage Allowance -->
                <div class="space-y-1.5">
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-600">Baggage Allowance</label>
                    <input type="text" name="baggage_allowance" value="{{ old('baggage_allowance', $flight->baggage_allowance) }}" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold">
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <select name="status" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold">
                        @foreach(['Scheduled', 'Boarding', 'Departed', 'Arrived', 'Cancelled'] as $st)
                            <option value="{{ $st }}" {{ $flight->status === $st ? 'selected' : '' }}>{{ $st }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
